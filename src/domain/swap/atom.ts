...
export const balanceAtom = atomWithQuery(get => {
  return {
    queryKey: queryKeys.balance.metaMask(get(tokenInAddressAtom), get(balanceFetchKey)),
...
export const targetCurrencyInUSDCAtom = atomWithQuery(get => ({
  queryKey: queryKeys.currency.byCoinId('usd-coin', get(targetCurrencyAtom)),
  queryFn: async ({ queryKey }: ContextFromQueryKey<typeof queryKeys.currency.byCoinId>) => {
    const [_, params] = queryKey;

    const { data } = await axiosInstance.get<number>(`${config.commonApiEndpoint}/css/currency`, {
      params,
    });
...
export const tokenPriceListAtom = atomWithQuery(get => ({
  queryKey: queryKeys.balance.byAddress(get(fromTokenEndpoint)),
...
export const getTokenOutDenomAtom = atom<(amount: string) => number>(get => {
